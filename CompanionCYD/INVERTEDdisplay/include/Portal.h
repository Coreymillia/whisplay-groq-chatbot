...
static bool ccConnect(bool forcePortal = false) {
  ...
  // Keep the saved config even if WiFi is slow or temporarily unavailable.
  // The main UI will keep retrying in the background.
  // Do not bounce the user back into setup just because the Pi companion API
  // misses a boot-time probe. Keep the saved config, boot normally, and let
  // the dashboard retry polling in the background.
  ccProbeApiReachable();
  return WiFi.status() == WL_CONNECTED;
}
